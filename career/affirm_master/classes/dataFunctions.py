...
#Function 2A: Load Facility data from a csv file into a dictionary file
def load_facilities_dict(FILE_PATH):
    with open(FILE_PATH + 'facilities.csv') as file:
        facilityReader = csv.reader(file)
        ...
        for currentLine in facilityReader:
            ...
            current_facility = Facility(id, bank_id, amount, interest_rate)
            facility_id = int(id)
            all_facilities_dict[facility_id] = current_facility
            #all_facilities.append(current_facility)
        
        return all_facilities_dict


#Function 2B: Load Covenant data from a csv file
def create_covenant_dicts(FILE_PATH):
    #banned_states_dict = {}
    max_default_dict = {}
    #max_default_dict[1] = .06
    #max_default_dict[2] = .09
    ...
    with open(FILE_PATH + 'covenants.csv') as file:
        ...
        for currentLine in covenantReader:
            facility_id = int(currentLine[0])
            max_default_likelihood = currentLine[1]
            bank_id = currentLine[2]
            banned_state = currentLine[3]
            print(facility_id, " ", banned_state, " ", max_default_likelihood)
            print(max_default_dict)

            if max_default_likelihood != "":
                max_default_dict[facility_id] = max_default_likelihood
            
            print(max_default_dict)
            print()

        return max_default_dict



def create_covenant_banned_states(FILE_PATH):
    banned_states_dict = {}
