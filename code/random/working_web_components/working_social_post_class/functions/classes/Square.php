...
class Square {
	...
	//If user is in square return 1 (true) otherwise return 0 (false)
	public function checkSquareUserName($squareId, $user_name) {
		global $conn;
		$square_id = $squareId;
		...
		while($row_square_friend = mysqli_fetch_array($result_square_friend)) {
			...
			//echo "<br />";
			$all_square_users[] = $row_square_friend['user_name'];
		}

		if (in_array($user_name, $all_square_users)) {
			$this->userInSquare = 1;
		} else {
			$this->userInSquare = 0;
		}
